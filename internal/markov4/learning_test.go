...
func BenchmarkLearning(b *testing.B) {
	m := MakeMarkov("")
	for i := 0; i < b.N; i++ {
		m.LearnString("Hello, World!")
	}
}

func TestSimpleLearning(t *testing.T) {
	assert.Equal(t, 1, 1)

	mk := MakeMarkov("ohgod")
	mk.LearnString("@Dungar how are you?")
	mk.LearnString("Does it start at zero?")
}

func TestStoredTweetLearning(t *testing.T) {
